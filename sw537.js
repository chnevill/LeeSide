...
self.addEventListener('install',e=>{
  e.waitUntil(
    caches.open(CACHE).then(c=>
      ...
      Promise.all(URLS.map(url=>fetch(url,{cache:'reload'}).then(res=>{
        // A failed/unexpected response (e.g. a transient 404 from GitHub's CDN
        // not yet having caught up right after a deploy) must never get cached -
        // throwing here fails this install attempt entirely, so the browser
        // discards it and keeps the current, working service worker in control
        // rather than silently locking in a broken page. The install gets
        // retried automatically on the next check, by which point the CDN has
        // normally settled.
        if(!res.ok)throw new Error('Install fetch failed for '+url+': '+res.status);
        return c.put(url,res);
      })))
    )
    // Deliberately no self.skipWaiting() here. Without it, a newly-installed
    // SW sits quietly in the background and only takes over once every open
    // instance of the app has been fully closed - i.e. only on a genuine cold
    // start, never while someone might be mid-flight-log or mid-note with
    // unsaved input open. The controllerchange handler in index.html still
    // exists as a safety net for edge cases, but in normal use it should now
    // only ever fire on a fresh load anyway, where reloading is a no-op.
  );
});
...
// Lets the page trigger this worker to activate immediately, at a moment it
// has determined is safe (no open overlay/form) - rather than only ever
// happening passively once every open instance of the app closes on its
// own, which iOS in particular may not reliably ever report through normal
// app-switching. skipWaiting() only has an effect on a worker that's
// actually waiting, so this is harmless if called on one that's already
// active or still installing.
...
self.addEventListener('fetch',e=>{
  ...
  const isGuideHtml=url.pathname==='/LeeSide/uplift-guide.html';
  const isScreenshot=url.pathname.startsWith('/LeeSide/screenshots/');
